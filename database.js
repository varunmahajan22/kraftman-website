...
if (!db.certifications.length) {
  ...
  const certs = [
    ...
    { title: 'GMI Certified', body: 'G7 Master Qualification for colour accuracy and brand consistency across all print runs.', image_url: '', badge_icon: 'fa-award', sort_order: 2, is_active: 1 },
  ];
  certs.forEach(c => {
...
if (!db.capabilities.length) {
  ...
  const caps = [
    ...
    { title: 'Quality Assurance', description: 'Multi-stage inspection, burst strength & compression testing, transit & drop simulation, ISO / BRC compliance, Six Sigma & lean manufacturing.', icon: 'fa-shield-alt', image_url: '', sort_order: 4, is_active: 1 },
  ];
  caps.forEach(c => {
